  ...
  jobs: [
    {
      ...
      city: "Belo Horizonte",
      description:
        "Working mostly with automation tests using Specflow and C# but also helping the PDMs writing the stories and the BDD scenarios.Additionally, I have been working executing functional tests on PETE Client solution"
    },
    {
...
function displayWork() {
  for (jobs in work.jobs) {
    ...
    var formattedEmployer = HTMLworkEmployer.replace(
      "%data%",
      work.jobs[jobs].employer
    );
    var formattedWorkTitle = HTMLworkTitle.replace(
      "%data%",
      work.jobs[jobs].position
    );
...
var buttonI18n = $(
  '<button type = "button" value = "Internationalize" id = "internationalizeButton" onclick = "inName(name)">Internationalize</button>'
);
...
function inName(name) {
  var nameComplete = [];
  nameComplete = name.split(" ");

  var firstName = nameComplete[0];
  var lastName = nameComplete[1];
